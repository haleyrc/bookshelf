...
func TestLibraryService_AddBook(t *testing.T) {
	...
	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			...
			store := &mockStore{
				CreateBookFn: func(_ context.Context, book *library.Book) error {
					book.ID = 123
					return nil
				},
			}
			...
			if args.Book.Title != tc.Request.Title {
				t.Errorf("expected CreateBook to be called with title %q, but got %q", tc.Request.Title, args.Book.Title)
			}
			if args.Book.Author != tc.Request.Author {
				t.Errorf("expected CreateBook to be called with author %q, but got %q", tc.Request.Author, args.Book.Author)
			}
...
type mockStore struct {
	...
	CreateBookFn func(ctx context.Context, book *library.Book) error
...
func (ms *mockStore) CreateBook(ctx context.Context, book *library.Book) error {
	...
	return ms.CreateBookFn(ctx, book)
}
